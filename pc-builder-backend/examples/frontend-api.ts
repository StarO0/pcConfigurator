...
const storage = {
  ...
  saveBuildToken(buildId: string, token: string | null) {
    if (token) localStorage.setItem(`build_token:${buildId}`, token);
  },
  buildToken: (buildId: string) => localStorage.getItem(`build_token:${buildId}`),
};
...
export const pcBuilderApi = {
  async generateBuilds(
    prompt: string,
    options: { budget?: number; currency?: string; basketMode?: "cheapest" | "fewest_stores" | "balanced" } = {},
  ) {
    const result = await request<{ requirements: unknown; builds: GeneratedBuild[]; cached: boolean }>(
      "/builds/generate",
      {
        method: "POST",
        body: JSON.stringify({
    ...
    result.builds.forEach((build) => storage.saveBuildToken(build.id, build.access_token));
    return result;
  },

  getBuild(buildId: string) {
    return request<Build>(`/builds/${buildId}`, { buildToken: storage.buildToken(buildId) ?? undefined });
  },

  getReplacementOptions(buildId: string, category: string) {
    return request<Array<{ product: Product; is_compatible: boolean; projected_total: string }>>(
      `/builds/${buildId}/replacement-options/${category}`,
  ...
  replaceComponent(build: Build, category: string, productId: string) {
    return request<Build>(`/builds/${build.id}/components/${category}`, {
      ...
      body: JSON.stringify({ product_id: productId, expected_version: build.version, basket_mode: "balanced" }),
    });
  },

  compare(left: GeneratedBuild, right: GeneratedBuild) {
    return request<{ left: Build; right: Build; differences: unknown[]; total_price_delta: string }>(
      "/builds/compare",
      {
        method: "POST",
        body: JSON.stringify({
  ...
  async logout(allSessions = false) {
    await request("/auth/logout", {
      method: "POST",
      body: JSON.stringify({ refresh_token: storage.refresh(), all_sessions: allSessions }),
    });
    storage.clear();
  },
  ...
  searchProducts(params: URLSearchParams) {
    return request<{ items: Product[]; total: number; limit: number; offset: number }>(`/products?${params}`);
  },

  createPriceAlert(productId: string, targetPrice: number, currency = "PLN") {
    return request("/price-alerts", {
      method: "POST",
      body: JSON.stringify({ product_id: productId, target_price: targetPrice, currency }),
    });
  },
};
